...
export default class Map {
  ...
  static async getPlaceNameByCoordinate(latitude, longitude) {
    try {
      ...
      if (!MAP_SERVICE_API_KEY || MAP_SERVICE_API_KEY === 'n91UI5X4lpIhuGYA6T9F') {
  ...
  changeCamera(coordinate, zoomLevel = null) {
    ...
    try {
      ...
      if (!zoomLevel) {
        this.#map.setView(defaultCoordinate, this.#zoom);
        return;
      }
  ...
  static async build(selector, options = {}) {
    ...
    const defaultCoordinate = [-7.9781, 112.5609]; // Koordinat Malang
